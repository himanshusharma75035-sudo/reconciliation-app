...
| A GROUP-level aggregate status painted onto a per-TRANSACTION row | Once the all-or-nothing per-group status (gotcha above) gained a real sub-group matcher, a group could be `partial` (some legs settled, some open). One report sheet still labelled each transaction row by a whole-group `{group_id: status}` lookup — so a fully-settled ₹1.6L withdrawal read "Partial" while the app's per-leg view showed it "Matched". Operator saw the report and the app disagree on the SAME row | `partial` is a GROUP-DAY aggregate and must NEVER tag an individual row. Every per-transaction surface must resolve per-leg through the ONE shared record of which amounts paired (`matched_amounts`): consume a per-`(date,group)` Counter as rows emit so a settled leg reads Matched, a residual reads Unmatched, and duplicates split one-each. The engine, the app view, and EVERY report sheet must call the same resolver — a whole-group status map is only ever valid where there's genuinely one row per group. Also decide what the column means for rows the concept doesn't apply to (a settlement status on a non-settled deposit row) — return N/A (`—`), not the group label, or the aggregate leaks back in. Verify with the invariant: per-row Matched count == `sum(len(matched_amounts))` over the groups |
| Hardwiring one LLM vendor into the app | The AI recon engines called Anthropic's SDK directly, so the product was locked to one vendor's key, pricing and availability — no room for a cheaper model, an on-prem/open-source model, or a customer's own provider. | Abstract to TWO provider families and you reach almost everything: the vendor's native API, plus the OpenAI Chat-Completions API with a configurable `base_url`. That one setting reaches OpenAI, Azure, OpenRouter, Groq, Together, DeepSeek, Mistral, Gemini (its OpenAI-compatible endpoint) AND every self-hosted / open-source server (Ollama, vLLM, LM Studio, LocalAI). Map the native tool schema (`input_schema`) → an OpenAI function (`parameters`), force the tool via `tool_choice`, and add a JSON-in-content fallback for weak models that ignore forced tools. Make it backward-compatible — provider defaults to the incumbent and the key falls back to the legacy setting, so no existing install reconfigures — and leave any AGENTIC loop on the vendor whose tool-runner it depends on (only the single-shot tool calls need to be portable). Allow a keyless local endpoint (self-hosted needs no API key). |
| An auto-close classification that starves a later matching pass | Ingest bucketed an ambiguous bank CREDIT as an auto-closed category ('fund_transfer') — but a downstream netting pass that pairs a refund round-trip (DR↔CR by tracking) acts ONLY on 'unmatched' rows. So the auto-classification pre-empted the netting: a genuine refund credit sat closed-but-wrong while its debit stayed open forever, and no amount of re-running recon fixed it (the credit was never 'unmatched'). | When a heuristic auto-closes rows into a bucket, some will be wrong, and a later pass that only looks at 'unmatched' can never rescue them. Give the operator a targeted ESCAPE HATCH: reopen the mis-bucketed row to 'unmatched' and immediately re-run the specific pass that should own it (here run_bank_reversal_match). Keep it operator-driven (they know which of N auto-closed rows are really refunds — measure the fixable subset: 290 of 959 had a same-tracking open debit) and make it a no-op for the correctly-classified majority. Reuse the existing pass rather than a new matcher — the pass already ignored row_type for exactly this reason |
| A read-time overlay applied in ONE view but not the independent report engine | Operator manual pairs (a read-time overlay keyed by stable business key) were applied in the live/unified view but NOT in the separate `reconcile()` engine that builds the downloadable operator workbooks — that engine re-derives its own matches by ref and never consulted the overlay. So an operator manually matched dozens of rows, saw them Matched in the app, but the report he downloaded still printed them Unmatched ("my manual matches don't show in the report") | A manual/override overlay must be applied in EVERY surface that classifies the same rows — including any independent report/aggregation engine that computes matches its own way, not just the live view. Apply it read-time in that engine too (compute each row's overlay key the SAME way the overlay was written; a lazy import breaks the core→routes cycle), flipping open→matched and moving counts out of the open buckets so the summary stays consistent. Make it a NO-OP when no overlays exist so every existing report test stays byte-identical, then add one test WITH an overlay. Same family as "a new closed disposition must reach the report layer, not just the live view" |
| An open-items picker gated on `unified-open AND report-open` | The pair-picker's bank side kept rows that were open in BOTH the unified view AND the reconciliation report — added to NARROW the raw unified set (the report reconciles some rows the unified view leaves open). But the reverse also happens: a bank CREDIT read `Matched` in the unified view via a WEAK `(csp/ko, amount)` P03 overlay while the report (matching by 20-digit ref) correctly left it Unmatched. The AND-gate then HID a real report-open item from the operator — "unmatched in the report but missing from Manual Match" | When one source is the designated source of truth (here the report, for the bank side), the picker should DEFER to it, not AND with a second view that can disagree in both directions. But don't blanket-defer: still exclude rows reliably reconciled by a trustworthy process (P01 settlement / P02 ref-match, distinguished by `result_process`) and manual pairs (which the report can't see). Only the weak-overlay "matches" defer to the report. Verify both directions with tests: a genuinely P02-matched row stays hidden; a weak-overlay-matched but report-open row shows |
| Extending a two-legged "A↔B" overlay to same-type pairing (A↔A) | A manual-pair overlay stored a `bank_*` leg and a `data_*` leg and split the read-time apply purely on `entry.side` (`if side=="bank": by_bank else by_data`). Operators then needed to pair two SAME-side rows (KO Withdrawal ↔ KO Deposit, both internal). The side-split made a same-side pair unresolvable, and the per-side uniqueness sets (`used_bank`/`used_data`) let one row be reused as the other leg | If the two legs' business keys live in DISJOINT namespaces (here `bank`/`bankset` vs `txr`/`kow`/`kod` prefixes), you don't need a schema change: collapse the two per-side lookups into ONE `{key: [(pair, leg)]}` map consumed once per `(pair, leg)`, and merge the uniqueness sets into one `used_keys` (a row is at most one leg of one pair, regardless of side). Add a self-pair guard (`bd.key == dd.key`). The `bank_*`/`data_*` column names become generic "leg A / leg B" — reuse them; a migration would be pure churn. Existing A↔B pairs stay byte-identical because the namespaces never collide |
| A one-time UPPERCASE legacy relabel vs a new lowercase status, on a case-INSENSITIVE DB | A startup migration folded legacy `IN ('PENDING','PARTIAL','EXCESS') → 'unmatched'`. Later a sub-pair matcher introduced a legitimate lowercase `'partial'`. On MySQL prod (`utf8mb4_unicode_ci`, case-insensitive) the literal `'PARTIAL'` ALSO matched `'partial'`, so **every restart silently flipped all sub-pair partials to `'unmatched'`** — the status never survived a reboot. SQLite dev (case-sensitive) never reproduced it, so tests were green. Only a data-integrity sweep caught it | String comparisons in SQL inherit the COLUMN's collation, not Python's `==`. When a case-insensitive DB must distinguish tokens that differ only by case, force a binary/case-sensitive match on that dialect (`col.collate("utf8mb4_bin")` on MySQL; skip on SQLite which has no such collation). Two broader rules: never let a new enum value be a case-variant of a legacy one; and a "one-time idempotent" migration that runs every startup is a standing hazard — the day someone adds a colliding value it starts corrupting live data with zero new code. Recovery is cheap only because the per-amount contract (`matched_amounts`) was untouched — re-running the engine recomputed the flipped status monotonically |
...
| Reclassifying a status in the live VIEW but not the REPORT/aggregation layer | A new "Failed (closed)" disposition moved failed txns out of "unmatched" in the unified view + picker, but the reconciliation + source workbooks still folded failures INTO "unmatched" — so the same rows were counted two ways and the app vs the report diverged on the headline number (0 vs 1,740 on one date — a 26× gap that reads as "reports out of sync") | A new terminal/closed disposition must reach EVERY surface that buckets that status — the live views AND the report/aggregation/analytics builders — in the SAME change, or you manufacture a cross-surface desync. Verify by making the report's bucket equal the app's on real data (here 07-29 report==app exactly after the fix). Caveat: where a report ADDITIONALLY re-derives the match (by reference) vs the app deferring to the result table, a small residual persists that a status relabel can't close — flag it as the pre-existing re-derive gap, don't chase pixel-parity |
